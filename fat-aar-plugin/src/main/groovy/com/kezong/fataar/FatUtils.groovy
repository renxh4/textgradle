...
class FatUtils {
    ...
    def static printDebug(String msg) {
        println("[debug] ${msg}")
    }
    ...
    def static searchSo(File file, List<File> lists) {
        if (file != null && file.isDirectory()) {
            findFilesBySuffix(file, lists, ".so")
        }
    }

    static void findFilesBySuffix(File file, List<File> lists, String suffix) {
        if (file.isFile() && !file.isDirectory() && file.name.endsWith(suffix)) {
            if (lists != null) {
                lists.add(file)
            }
            return
        ...
        if (file.isDirectory()) {
            File[] files = file.listFiles()
            for (int i = 0; i < files.length; i++)
                findFilesBySuffix(files[i], lists, suffix)
        }
    }
