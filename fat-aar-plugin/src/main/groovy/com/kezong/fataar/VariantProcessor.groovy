...
class VariantProcessor {
    ...
    private Collection<String> mJniSoFiles = new ArrayList<>()
    ...
    private static void printEmbedArtifacts(Collection<ResolvedArtifact> artifacts,
        ...
        for (String name: moduleNames) {
            FatUtils.printDebug("each moduleName = ${name}")
        }
        dependencies.each { dependency ->
            FatUtils.printDebug("dependency.moduleName=${dependency.moduleName}")
    ...
    private TaskProvider configureReBundleAarTask(TaskProvider bundleTask) {
        ...
        bundleTask.configure { it ->
            if (FatUtils.compareVersion(mProject.gradle.gradleVersion, "5.1") >= 0) {
                ...
                FatUtils.printDebug("aarOutputFile = ${aarOutputFile.absolutePath}")
            } else {
        ...
        FatUtils.printDebug("reBundleAar, taskName=${taskName}")
        TaskProvider task = mProject.getTasks().register(taskName, Zip.class) {
            ...
            FatUtils.printDebug("reBundleAar, taskName=${taskName}, executing.")
        ...
        for (final ResolvedArtifact artifact in artifacts) {
            if (FatAarPlugin.ARTIFACT_TYPE_JAR == artifact.type) {
                if (artifact.file.exists()) {
                    ...
                    FatUtils.printDebug("[zzzzzzzz][jar] group=${gp}:${na}, size=${size}")
                }
            ...
            doFirst {
                for (archiveLibrary in mAndroidArchiveLibraries) {
                    if (archiveLibrary.jniFolder != null && archiveLibrary.jniFolder.exists()) {
                        mProject.android.sourceSets.each {
                            if (it.name == mVariant.name) {
                                FatUtils.printDebug("[mergeJniLibsTask] group=" + archiveLibrary.group + ":" + archiveLibrary.name + ", jniFolder=" + archiveLibrary.jniFolder)
                                if (mProject.fataar.pickFirstSo) {
                                    processPickFirstSo(archiveLibrary.jniFolder, archiveLibrary)
                                }
    ...
    private void processPickFirstSo(File jniFolder, AndroidArchiveLibrary archiveLibrary) {
        FatUtils.printDebug("[PickFirstSo] start parsing jniFolder=${jniFolder.absolutePath}")
        List<File> soList = new ArrayList<>();
        FatUtils.searchSo(jniFolder, soList)
        for (File so : soList) {
            String relativePath = so.absolutePath.substring(jniFolder.absolutePath.length() + 1, so.absolutePath.length())
            String[] spls = relativePath.split("/")
            String abiName = spls[0]
            String soName = spls[1]
            FatUtils.printDebug("[PickFirstSo] cur so.name=${soName} group=${archiveLibrary.group}:${archiveLibrary.name} ; jniFolder=" + archiveLibrary.jniFolder)
            if (mJniSoFiles.contains(relativePath)) {
                so.delete()
                FatUtils.printDebug("[PickFirstSo] delete dumplicate so :${so.name}, path:${relativePath}")
            }
            if (mProject.fataar.abiFilter != null) {
                boolean isContain = false
                for (String abi: mProject.fataar.abiFilter) {
                    if (abiName == abi) {
                        isContain = true
                    }
                }
                if (!isContain) {
                    so.delete()
                    FatUtils.printDebug("[PickFirstSo] abiFilter delete so :${so.name}, path:${relativePath}")
                }
            }
            if (mProject.fataar.excludeSo != null) {
                for (String aSo: mProject.fataar.excludeSo) {
                    if (aSo == soName) {
                        so.delete()
                        FatUtils.printDebug("[PickFirstSo] excludeSo delete so :${so.name}, path:${relativePath}")
                    }
                }
            }
            mJniSoFiles.add(relativePath)

//            if (mJniSoFiles.contains(relativePath)) {
//                so.delete()
//                FatUtils.printDebug("[PickFirstSo] delete so :${so.name}, path:${relativePath}")
//            } else if (relativePath.contains("x86")) {
//                so.delete()
//                FatUtils.printDebug("[PickFirstSo] delete x86 so :${so.name}, path:${relativePath}")
//            } else if (relativePath.contains("armeabi")) {
//                so.delete()
//                FatUtils.printDebug("[PickFirstSo] delete armeabi/armeabi-v7a so :${so.name}, path:${relativePath}")
//            } else {
//                mJniSoFiles.add(relativePath)
//            }
        }
