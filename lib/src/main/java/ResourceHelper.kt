...
class ResourceHelper {
    ...
    private fun compileResourcesWithout() {
        ...
        if (ret.errorResult.size > 0) {
            WinkLog.d("Compile resources error.")
        }
    }
    ...
    fun packageResources() {
        ...
        val app = Settings.getProjectTreeRoot()!!.name
        ...
        val localScript = """
            ...
            cp -R $lastPath/$app/build/intermediates/merged_assets/${Settings.getVariantName()}/out/. $lastPath/.idea/${Constant.TAG}/tempResFolder/assets
